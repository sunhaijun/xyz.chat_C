import { Google, REQUEST_TIMEOUT_MS, ServiceProvider } from "@/app/constant";
...
export class GeminiProApi implements LLMApi {
  ...
  async chat(options: ChatOptions): Promise<void> {
    ...
    let baseUrl = accessStore.rpsEndPoint;
    ...
    try {
      ...
      if (isApp) {
        baseUrl += `?key=${accessStore.authToken}`;
      }
      const chatPayload = {
        method: "POST",
        body: JSON.stringify(requestPayload),
        signal: controller.signal,
        headers: getHeaders(ServiceProvider.Google),
      };
