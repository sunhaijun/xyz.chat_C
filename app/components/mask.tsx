...
import { ModelCodePrefix } from "../constant";
    ...
    props.updateContext((context) => {
      ...
      if (images.length > 0) {
        ...
        for (const img of images) {
          ...
          if (context[i].model?.startsWith(ModelCodePrefix.Anthropic)) {
            // Claude-3-*
            newContext.push({
              type: "image",
              source: {
                type: "base64",
                media_type: img.type, // use the type from the image object
                data: img.dataUrl, // use the dataUrl from the image object
              },
            });
          } else {
            newContext.push({
              type: "image_url",
              image_url: {
                url: img.dataUrl, // use the dataUrl from the image object
              },
            });
          }
        }
